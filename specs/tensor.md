...
alias: tensor trace compression, tensor compression, streaming sumcheck
---
# tensor

tensor decomposition of structured nox traces. prover memory: O(N) → O(√N). prover time: O(N log N) → O(N) streaming. mobile devices become first-class provers.
...
- tensor: 32M ops (2x more but streaming), ~32 KB memory (1000x less)
...
total: r × N ≈ 32N hashes (vs N log N for standard Merkle-based PCS)
